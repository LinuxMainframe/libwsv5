...
This is non-\/blocking -\/ it returns immediately. The actual connection happens in a background thread. Check obsws\+\_\+get\+\_\+state() to see when it reaches CONNECTED.
...
\end{DoxyReturn}
\begin{DoxyNote}{Note}
The config structure is copied internally, so you can free or reuse it after calling obsws\+\_\+connect(). 

The returned handle is opaque -\/ use it with other obsws\+\_\+$\ast$ functions. 

Connection attempt happens asynchronously -\/ wait for state callback or call obsws\+\_\+get\+\_\+state() to check when it\textquotesingle{}s ready. 
